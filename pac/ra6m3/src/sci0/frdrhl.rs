...
///Field `RDAT` reader - Serial receive data(Valid only in asynchronous mode(including multi-processor) or clock synchronous mode, and FIFO selected)
...
impl R {
    ///Bits 0:8 - Serial receive data(Valid only in asynchronous mode(including multi-processor) or clock synchronous mode, and FIFO selected)
    #[inline(always)]
    pub fn rdat(&self) -> RDAT_R {
        RDAT_R::new(self.bits & 0x01ff)
    }
    ///Bit 9 - Multi-processor bit flag(Valid only in asynchronous mode with SMR.MP=1 and FIFO selected) It can read multi-processor bit corresponded to serial receive data(RDATA\[8:0\])
    #[inline(always)]
    pub fn mpb(&self) -> MPB_R {
        MPB_R::new(((self.bits >> 9) & 1) != 0)
    }
    ///Bit 10 - Receive data ready flag(It is same as SSR.DR)
    #[inline(always)]
    pub fn dr(&self) -> DR_R {
    ...
    ///Bit 13 - Overrun error flag(It is same as SSR.ORER)
    #[inline(always)]
    pub fn orer(&self) -> ORER_R {
        ORER_R::new(((self.bits >> 13) & 1) != 0)
    }
    ///Bit 14 - Receive FIFO data full flag(It is same as SSR.RDF)
    #[inline(always)]
