...
///Field `WINDEX` reader - Value of USB request wIndex Finction controller selected : read-only Host controller selected : read-write
pub type WINDEX_R = crate::FieldReader<u16>;
///Field `WINDEX` writer - Value of USB request wIndex Finction controller selected : read-only Host controller selected : read-write
