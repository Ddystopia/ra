...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum DRPD_A {
    ///0: Pulling down the line is disabled.
    _0 = 0,
    ///1: Pulling down the line is enabled.
    _1 = 1,
}
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum HSE_A {
    ///0: High-speed operation is disabled.(When the function controller function is selected: Full speed, When the host controller function is selected: Full/low speed)
    _0 = 0,
    ///1: High-speed operation is enabled (the controller detects the communication speed).
    _1 = 1,
}
...
impl HSE_R {
    ///Get enumerated values variant
    #[inline(always)]
    pub const fn variant(&self) -> HSE_A {
        match self.bits {
            false => HSE_A::_0,
            true => HSE_A::_1,
        }
...
pub type HSE_W<'a, REG> = crate::BitWriter<'a, REG, HSE_A>;
impl<'a, REG> HSE_W<'a, REG>
where
    REG: crate::Writable + crate::RegisterSpec,
{
    ///High-speed operation is disabled.(When the function controller function is selected: Full speed, When the host controller function is selected: Full/low speed)
...
///Field `CNEN` reader - Single End Receiver Enable
...
///Field `CNEN` writer - Single End Receiver Enable
pub type CNEN_W<'a, REG> = crate::BitWriter<'a, REG, CNEN_A>;
impl<'a, REG> CNEN_W<'a, REG>
where
    REG: crate::Writable + crate::RegisterSpec,
{
    ///Single end receiver operation is disabled.
...
impl R {
    ...
    ///Bit 8 - Single End Receiver Enable
    #[inline(always)]
...
impl W {
    ...
    ///Bit 7 - High-Speed Operation Enable
    #[inline(always)]
    pub fn hse(&mut self) -> HSE_W<'_, SYSCFG_SPEC> {
        HSE_W::new(self, 7)
    }
    ///Bit 8 - Single End Receiver Enable
    #[inline(always)]
    pub fn cnen(&mut self) -> CNEN_W<'_, SYSCFG_SPEC> {
        CNEN_W::new(self, 8)
    }
}
