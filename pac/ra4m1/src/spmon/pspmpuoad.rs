///Register `PSPMPUOAD` reader
pub type R = crate::R<PSPMPUOAD_SPEC>;
///Register `PSPMPUOAD` writer
...
impl W {
    ///Bit 0 - Operation after detection
    #[inline(always)]
    pub fn oad(&mut self) -> OAD_W<'_, PSPMPUOAD_SPEC> {
        OAD_W::new(self, 0)
    }
    ///Bits 8:15 - Key Code The data written to these bits are not stored.
    #[inline(always)]
    pub fn key(&mut self) -> KEY_W<'_, PSPMPUOAD_SPEC> {
        KEY_W::new(self, 8)
    }
}
...
///`read()` method returns [`pspmpuoad::R`](R) reader structure
impl crate::Readable for PSPMPUOAD_SPEC {}
///`write(|w| ..)` method takes [`pspmpuoad::W`](W) writer structure
