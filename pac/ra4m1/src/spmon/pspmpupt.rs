///Register `PSPMPUPT` reader
pub type R = crate::R<PSPMPUPT_SPEC>;
///Register `PSPMPUPT` writer
...
///Field `PROTECT` reader - Protection register (PSPMPUAC, PSPMPUSA and PSPMPUSE)
...
///Field `PROTECT` writer - Protection register (PSPMPUAC, PSPMPUSA and PSPMPUSE)
...
///Field `KEY` writer - Key Code The data written to these bits are not stored.
...
impl R {
    ///Bit 0 - Protection register (PSPMPUAC, PSPMPUSA and PSPMPUSE)
    #[inline(always)]
...
impl W {
    ///Bit 0 - Protection register (PSPMPUAC, PSPMPUSA and PSPMPUSE)
    #[inline(always)]
    pub fn protect(&mut self) -> PROTECT_W<'_, PSPMPUPT_SPEC> {
        PROTECT_W::new(self, 0)
    }
    ///Bits 8:15 - Key Code The data written to these bits are not stored.
    #[inline(always)]
    pub fn key(&mut self) -> KEY_W<'_, PSPMPUPT_SPEC> {
        KEY_W::new(self, 8)
    }
}
...
///`read()` method returns [`pspmpupt::R`](R) reader structure
impl crate::Readable for PSPMPUPT_SPEC {}
///`write(|w| ..)` method takes [`pspmpupt::W`](W) writer structure
