...
///Field `TSL` reader - Time Stamp Higher Byte Bits TSL\[7:0\] store the counter value of the time stamp when received messages are stored in the mailbox.
pub type TSL_R = crate::FieldReader;
///Field `TSL` writer - Time Stamp Higher Byte Bits TSL\[7:0\] store the counter value of the time stamp when received messages are stored in the mailbox.
pub type TSL_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
///Field `TSH` reader - Time Stamp Lower Byte Bits TSH\[7:0\] store the counter value of the time stamp when received messages are stored in the mailbox.
pub type TSH_R = crate::FieldReader;
///Field `TSH` writer - Time Stamp Lower Byte Bits TSH\[7:0\] store the counter value of the time stamp when received messages are stored in the mailbox.
pub type TSH_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
impl R {
    ///Bits 0:7 - Time Stamp Higher Byte Bits TSL\[7:0\] store the counter value of the time stamp when received messages are stored in the mailbox.
    #[inline(always)]
    pub fn tsl(&self) -> TSL_R {
        TSL_R::new((self.bits & 0xff) as u8)
    }
    ///Bits 8:15 - Time Stamp Lower Byte Bits TSH\[7:0\] store the counter value of the time stamp when received messages are stored in the mailbox.
    #[inline(always)]
