...
///Field `ACCSTAT` reader - Error Access Status The status at the time of the error
...
///Field `ERRSTAT` reader - Bus Error Status When bus error assert, error flag occurs.
...
impl R {
    ///Bit 0 - Error Access Status The status at the time of the error
    #[inline(always)]
    pub fn accstat(&self) -> ACCSTAT_R {
        ACCSTAT_R::new((self.bits & 1) != 0)
    }
    ///Bit 7 - Bus Error Status When bus error assert, error flag occurs.
    #[inline(always)]
