...
/**ID frame select (Valid only in asynchronous mode(including multi-processor)

Value on reset: 1*/
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum IDSEL_A {
    ///0: Always compare data regardless of the value of the MPB bit.
    _0 = 0,
    ///1: Compare data when the MPB bit is 1 (ID frame) only.
    _1 = 1,
}
...
///Field `IDSEL` reader - ID frame select (Valid only in asynchronous mode(including multi-processor)
...
///Field `IDSEL` writer - ID frame select (Valid only in asynchronous mode(including multi-processor)
