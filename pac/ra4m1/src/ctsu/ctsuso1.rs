...
///Field `CTSURICOA` reader - CTSU Reference ICO Current Adjustment Current offset amount is CTSUSO ( 0 to 255 )
pub type CTSURICOA_R = crate::FieldReader;
///Field `CTSURICOA` writer - CTSU Reference ICO Current Adjustment Current offset amount is CTSUSO ( 0 to 255 )
pub type CTSURICOA_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
///Field `CTSUSDPA` reader - CTSU Base Clock Setting Operating clock divided by ( CTSUSDPA + 1 ) x 2
pub type CTSUSDPA_R = crate::FieldReader;
///Field `CTSUSDPA` writer - CTSU Base Clock Setting Operating clock divided by ( CTSUSDPA + 1 ) x 2
...
impl R {
    ///Bits 0:7 - CTSU Reference ICO Current Adjustment Current offset amount is CTSUSO ( 0 to 255 )
    #[inline(always)]
    pub fn ctsuricoa(&self) -> CTSURICOA_R {
        CTSURICOA_R::new((self.bits & 0xff) as u8)
    }
    ///Bits 8:12 - CTSU Base Clock Setting Operating clock divided by ( CTSUSDPA + 1 ) x 2
    #[inline(always)]
