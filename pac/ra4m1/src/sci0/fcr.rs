...
pub enum FM_A {
    ///0: Non-FIFO mode(Selects o TDR/RDR for communication)
    _0 = 0,
    ///1: FIFO mode (Selects to FTDRH and FTDRL/FRDRH and FRDRL for communication)
    _1 = 1,
}
...
/**Receive FIFO data trigger number (Valid only in asynchronous mode(including multi-processor) or clock synchronous mode)
...
///Field `RTRG` reader - Receive FIFO data trigger number (Valid only in asynchronous mode(including multi-processor) or clock synchronous mode)
...
///Field `RTRG` writer - Receive FIFO data trigger number (Valid only in asynchronous mode(including multi-processor) or clock synchronous mode)
