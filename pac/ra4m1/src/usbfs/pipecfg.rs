...
///Field `EPNUM` reader - Endpoint Number These bits specify the endpoint number for the selected pipe. Setting 0000b means unused pipe.
pub type EPNUM_R = crate::FieldReader;
///Field `EPNUM` writer - Endpoint Number These bits specify the endpoint number for the selected pipe. Setting 0000b means unused pipe.
...
pub enum TYPE_A {
    ///0: Pipe not used
    _00 = 0,
    ///1: Bulk transfer(PIPE1 and PIPE5) /Setting prohibited(PIPE6 to PIPE9)
    _01 = 1,
    ///2: Setting prohibited(PIPE1 and PIPE5) /Interrupt transfer(PIPE6 to PIPE9)
    _10 = 2,
    ///3: Isochronous transfer(PIPE1 and PIPE2) /Setting prohibited(PIPE3 to PIPE9)
    _11 = 3,
}
...
impl R {
    ///Bits 0:3 - Endpoint Number These bits specify the endpoint number for the selected pipe. Setting 0000b means unused pipe.
    #[inline(always)]
