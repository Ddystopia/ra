...
///Field `BCLR` writer - CPU Buffer Clear Note: Only 0 can be read.
...
impl R {
    ///Bits 0:8 - Receive Data Length Indicates the length of the receive data.
    #[inline(always)]
...
impl W {
    ///Bit 14 - CPU Buffer Clear Note: Only 0 can be read.
    #[inline(always)]
    pub fn bclr(&mut self) -> BCLR_W<'_, D1FIFOCTR_SPEC> {
        BCLR_W::new(self, 14)
    }
    ///Bit 15 - Buffer Memory Valid Flag
    #[inline(always)]
    pub fn bval(&mut self) -> BVAL_W<'_, D1FIFOCTR_SPEC> {
        BVAL_W::new(self, 15)
    }
}
