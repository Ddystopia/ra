...
///Field `BMREQUESTTYPE` reader - Request Type These bits store the USB request bmRequestType value.
pub type BMREQUESTTYPE_R = crate::FieldReader;
///Field `BMREQUESTTYPE` writer - Request Type These bits store the USB request bmRequestType value.
pub type BMREQUESTTYPE_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
///Field `BREQUEST` reader - Request These bits store the USB request bRequest value.
pub type BREQUEST_R = crate::FieldReader;
///Field `BREQUEST` writer - Request These bits store the USB request bRequest value.
pub type BREQUEST_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
impl R {
    ///Bits 0:7 - Request Type These bits store the USB request bmRequestType value.
    #[inline(always)]
    pub fn bmrequesttype(&self) -> BMREQUESTTYPE_R {
        BMREQUESTTYPE_R::new((self.bits & 0xff) as u8)
    }
    ///Bits 8:15 - Request These bits store the USB request bRequest value.
    #[inline(always)]
