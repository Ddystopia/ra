...
///Field `TEDGSEL` reader - I/O polarity switch Function varies depending on the operating mode.
pub type TEDGSEL_R = crate::BitReader;
///Field `TEDGSEL` writer - I/O polarity switch Function varies depending on the operating mode.
...
///Field `TOE` reader - AGTOn output enable
...
///Field `TOE` writer - AGTOn output enable
...
pub enum TIOGT_A {
    ...
    _00 = 0,
    ///1: Event is counted during polarity period specified for AGTEEn.
    _01 = 1,
    ///2: settings are prohibited.
    OTHERS = 2,
}
