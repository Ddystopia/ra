///Register `ADCMPANSR1` reader
pub type R = crate::R<ADCMPANSR1_SPEC>;
///Register `ADCMPANSR1` writer
pub type W = crate::W<ADCMPANSR1_SPEC>;
/**AN016 Select

Value on reset: 0*/
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum CMPCHA16_A {
    ///0: Excludes AN016 from the compare window A target range.
    _0 = 0,
    ///1: Includes AN016 from the compare window A target range.
    _1 = 1,
}
impl From<CMPCHA16_A> for bool {
    #[inline(always)]
    fn from(variant: CMPCHA16_A) -> Self {
        variant as u8 != 0
    }
}
///Field `CMPCHA16` reader - AN016 Select
pub type CMPCHA16_R = crate::BitReader<CMPCHA16_A>;
impl CMPCHA16_R {
    ///Get enumerated values variant
    #[inline(always)]
    pub const fn variant(&self) -> CMPCHA16_A {
        match self.bits {
            false => CMPCHA16_A::_0,
            true => CMPCHA16_A::_1,
        }
    }
    ///Excludes AN016 from the compare window A target range.
    #[inline(always)]
    pub fn is_0(&self) -> bool {
        *self == CMPCHA16_A::_0
    }
    ///Includes AN016 from the compare window A target range.
    #[inline(always)]
    pub fn is_1(&self) -> bool {
        *self == CMPCHA16_A::_1
    }
}
///Field `CMPCHA16` writer - AN016 Select
pub type CMPCHA16_W<'a, REG> = crate::BitWriter<'a, REG, CMPCHA16_A>;
impl<'a, REG> CMPCHA16_W<'a, REG>
where
    REG: crate::Writable + crate::RegisterSpec,
{
    ///Excludes AN016 from the compare window A target range.
    #[inline(always)]
    pub fn _0(self) -> &'a mut crate::W<REG> {
        self.variant(CMPCHA16_A::_0)
    }
    ///Includes AN016 from the compare window A target range.
    #[inline(always)]
    pub fn _1(self) -> &'a mut crate::W<REG> {
        self.variant(CMPCHA16_A::_1)
    }
}
/**AN017 Select

Value on reset: 0*/
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum CMPCHA17_A {
    ///0: Excludes AN017 from the compare window A target range.
    _0 = 0,
    ///1: Includes AN017 from the compare window A target range.
    _1 = 1,
}
impl From<CMPCHA17_A> for bool {
    #[inline(always)]
    fn from(variant: CMPCHA17_A) -> Self {
        variant as u8 != 0
    }
}
///Field `CMPCHA17` reader - AN017 Select
pub type CMPCHA17_R = crate::BitReader<CMPCHA17_A>;
impl CMPCHA17_R {
    ///Get enumerated values variant
    #[inline(always)]
    pub const fn variant(&self) -> CMPCHA17_A {
        match self.bits {
            false => CMPCHA17_A::_0,
            true => CMPCHA17_A::_1,
        }
    }
    ///Excludes AN017 from the compare window A target range.
    #[inline(always)]
    pub fn is_0(&self) -> bool {
        *self == CMPCHA17_A::_0
    }
    ///Includes AN017 from the compare window A target range.
    #[inline(always)]
    pub fn is_1(&self) -> bool {
        *self == CMPCHA17_A::_1
    }
}
///Field `CMPCHA17` writer - AN017 Select
pub type CMPCHA17_W<'a, REG> = crate::BitWriter<'a, REG, CMPCHA17_A>;
impl<'a, REG> CMPCHA17_W<'a, REG>
where
    REG: crate::Writable + crate::RegisterSpec,
{
    ///Excludes AN017 from the compare window A target range.
    #[inline(always)]
    pub fn _0(self) -> &'a mut crate::W<REG> {
        self.variant(CMPCHA17_A::_0)
    }
    ///Includes AN017 from the compare window A target range.
    #[inline(always)]
    pub fn _1(self) -> &'a mut crate::W<REG> {
        self.variant(CMPCHA17_A::_1)
    }
}
/**AN018 Select

Value on reset: 0*/
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum CMPCHA18_A {
    ///0: Excludes AN018 from the compare window A target range.
    _0 = 0,
    ///1: Includes AN018 from the compare window A target range.
    _1 = 1,
}
impl From<CMPCHA18_A> for bool {
    #[inline(always)]
    fn from(variant: CMPCHA18_A) -> Self {
        variant as u8 != 0
    }
}
///Field `CMPCHA18` reader - AN018 Select
pub type CMPCHA18_R = crate::BitReader<CMPCHA18_A>;
impl CMPCHA18_R {
    ///Get enumerated values variant
    #[inline(always)]
    pub const fn variant(&self) -> CMPCHA18_A {
        match self.bits {
            false => CMPCHA18_A::_0,
            true => CMPCHA18_A::_1,
        }
    }
    ///Excludes AN018 from the compare window A target range.
    #[inline(always)]
    pub fn is_0(&self) -> bool {
        *self == CMPCHA18_A::_0
    }
    ///Includes AN018 from the compare window A target range.
    #[inline(always)]
    pub fn is_1(&self) -> bool {
        *self == CMPCHA18_A::_1
    }
}
///Field `CMPCHA18` writer - AN018 Select
pub type CMPCHA18_W<'a, REG> = crate::BitWriter<'a, REG, CMPCHA18_A>;
impl<'a, REG> CMPCHA18_W<'a, REG>
where
    REG: crate::Writable + crate::RegisterSpec,
{
    ///Excludes AN018 from the compare window A target range.
    #[inline(always)]
    pub fn _0(self) -> &'a mut crate::W<REG> {
        self.variant(CMPCHA18_A::_0)
    }
    ///Includes AN018 from the compare window A target range.
    #[inline(always)]
    pub fn _1(self) -> &'a mut crate::W<REG> {
        self.variant(CMPCHA18_A::_1)
    }
}
/**AN019 Select

Value on reset: 0*/
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum CMPCHA19_A {
    ///0: Excludes AN019 from the compare window A target range.
    _0 = 0,
    ///1: Includes AN019 from the compare window A target range.
    _1 = 1,
}
impl From<CMPCHA19_A> for bool {
    #[inline(always)]
    fn from(variant: CMPCHA19_A) -> Self {
        variant as u8 != 0
    }
}
///Field `CMPCHA19` reader - AN019 Select
pub type CMPCHA19_R = crate::BitReader<CMPCHA19_A>;
impl CMPCHA19_R {
    ///Get enumerated values variant
    #[inline(always)]
    pub const fn variant(&self) -> CMPCHA19_A {
        match self.bits {
            false => CMPCHA19_A::_0,
            true => CMPCHA19_A::_1,
        }
    }
    ///Excludes AN019 from the compare window A target range.
    #[inline(always)]
    pub fn is_0(&self) -> bool {
        *self == CMPCHA19_A::_0
    }
    ///Includes AN019 from the compare window A target range.
    #[inline(always)]
    pub fn is_1(&self) -> bool {
        *self == CMPCHA19_A::_1
    }
}
///Field `CMPCHA19` writer - AN019 Select
pub type CMPCHA19_W<'a, REG> = crate::BitWriter<'a, REG, CMPCHA19_A>;
impl<'a, REG> CMPCHA19_W<'a, REG>
where
    REG: crate::Writable + crate::RegisterSpec,
{
    ///Excludes AN019 from the compare window A target range.
    #[inline(always)]
    pub fn _0(self) -> &'a mut crate::W<REG> {
        self.variant(CMPCHA19_A::_0)
    }
    ///Includes AN019 from the compare window A target range.
    #[inline(always)]
    pub fn _1(self) -> &'a mut crate::W<REG> {
        self.variant(CMPCHA19_A::_1)
    }
}
/**AN020 Select

Value on reset: 0*/
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum CMPCHA20_A {
    ///0: Excludes AN020 from the compare window A target range.
    _0 = 0,
    ///1: Includes AN020 from the compare window A target range.
    _1 = 1,
}
impl From<CMPCHA20_A> for bool {
    #[inline(always)]
    fn from(variant: CMPCHA20_A) -> Self {
        variant as u8 != 0
    }
}
///Field `CMPCHA20` reader - AN020 Select
pub type CMPCHA20_R = crate::BitReader<CMPCHA20_A>;
impl CMPCHA20_R {
    ///Get enumerated values variant
    #[inline(always)]
    pub const fn variant(&self) -> CMPCHA20_A {
        match self.bits {
            false => CMPCHA20_A::_0,
            true => CMPCHA20_A::_1,
        }
    }
    ///Excludes AN020 from the compare window A target range.
    #[inline(always)]
    pub fn is_0(&self) -> bool {
        *self == CMPCHA20_A::_0
    }
    ///Includes AN020 from the compare window A target range.
    #[inline(always)]
    pub fn is_1(&self) -> bool {
        *self == CMPCHA20_A::_1
    }
}
///Field `CMPCHA20` writer - AN020 Select
pub type CMPCHA20_W<'a, REG> = crate::BitWriter<'a, REG, CMPCHA20_A>;
impl<'a, REG> CMPCHA20_W<'a, REG>
where
    REG: crate::Writable + crate::RegisterSpec,
{
    ///Excludes AN020 from the compare window A target range.
    #[inline(always)]
    pub fn _0(self) -> &'a mut crate::W<REG> {
        self.variant(CMPCHA20_A::_0)
    }
    ///Includes AN020 from the compare window A target range.
    #[inline(always)]
    pub fn _1(self) -> &'a mut crate::W<REG> {
        self.variant(CMPCHA20_A::_1)
    }
}
/**AN021 Select

Value on reset: 0*/
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum CMPCHA21_A {
    ///0: Excludes AN021 from the compare window A target range.
    _0 = 0,
    ///1: Includes AN021 from the compare window A target range.
    _1 = 1,
}
impl From<CMPCHA21_A> for bool {
    #[inline(always)]
    fn from(variant: CMPCHA21_A) -> Self {
        variant as u8 != 0
    }
}
///Field `CMPCHA21` reader - AN021 Select
pub type CMPCHA21_R = crate::BitReader<CMPCHA21_A>;
impl CMPCHA21_R {
    ///Get enumerated values variant
    #[inline(always)]
    pub const fn variant(&self) -> CMPCHA21_A {
        match self.bits {
            false => CMPCHA21_A::_0,
            true => CMPCHA21_A::_1,
        }
    }
    ///Excludes AN021 from the compare window A target range.
    #[inline(always)]
    pub fn is_0(&self) -> bool {
        *self == CMPCHA21_A::_0
    }
    ///Includes AN021 from the compare window A target range.
    #[inline(always)]
    pub fn is_1(&self) -> bool {
        *self == CMPCHA21_A::_1
    }
}
///Field `CMPCHA21` writer - AN021 Select
pub type CMPCHA21_W<'a, REG> = crate::BitWriter<'a, REG, CMPCHA21_A>;
impl<'a, REG> CMPCHA21_W<'a, REG>
where
    REG: crate::Writable + crate::RegisterSpec,
{
    ///Excludes AN021 from the compare window A target range.
    #[inline(always)]
    pub fn _0(self) -> &'a mut crate::W<REG> {
        self.variant(CMPCHA21_A::_0)
    }
    ///Includes AN021 from the compare window A target range.
    #[inline(always)]
    pub fn _1(self) -> &'a mut crate::W<REG> {
        self.variant(CMPCHA21_A::_1)
    }
}
/**AN022 Select

Value on reset: 0*/
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum CMPCHA22_A {
    ///0: Excludes AN022 from the compare window A target range.
    _0 = 0,
    ///1: Includes AN022 from the compare window A target range.
    _1 = 1,
}
impl From<CMPCHA22_A> for bool {
    #[inline(always)]
    fn from(variant: CMPCHA22_A) -> Self {
        variant as u8 != 0
    }
}
///Field `CMPCHA22` reader - AN022 Select
pub type CMPCHA22_R = crate::BitReader<CMPCHA22_A>;
impl CMPCHA22_R {
    ///Get enumerated values variant
    #[inline(always)]
    pub const fn variant(&self) -> CMPCHA22_A {
        match self.bits {
            false => CMPCHA22_A::_0,
            true => CMPCHA22_A::_1,
        }
    }
    ///Excludes AN022 from the compare window A target range.
    #[inline(always)]
    pub fn is_0(&self) -> bool {
        *self == CMPCHA22_A::_0
    }
    ///Includes AN022 from the compare window A target range.
    #[inline(always)]
    pub fn is_1(&self) -> bool {
        *self == CMPCHA22_A::_1
    }
}
///Field `CMPCHA22` writer - AN022 Select
pub type CMPCHA22_W<'a, REG> = crate::BitWriter<'a, REG, CMPCHA22_A>;
impl<'a, REG> CMPCHA22_W<'a, REG>
where
    REG: crate::Writable + crate::RegisterSpec,
{
    ///Excludes AN022 from the compare window A target range.
    #[inline(always)]
    pub fn _0(self) -> &'a mut crate::W<REG> {
        self.variant(CMPCHA22_A::_0)
    }
    ///Includes AN022 from the compare window A target range.
    #[inline(always)]
    pub fn _1(self) -> &'a mut crate::W<REG> {
        self.variant(CMPCHA22_A::_1)
    }
}
/**AN023 Select

Value on reset: 0*/
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum CMPCHA23_A {
    ///0: Excludes AN023 from the compare window A target range.
    _0 = 0,
    ///1: Includes AN023 from the compare window A target range.
    _1 = 1,
}
impl From<CMPCHA23_A> for bool {
    #[inline(always)]
    fn from(variant: CMPCHA23_A) -> Self {
        variant as u8 != 0
    }
}
///Field `CMPCHA23` reader - AN023 Select
pub type CMPCHA23_R = crate::BitReader<CMPCHA23_A>;
impl CMPCHA23_R {
    ///Get enumerated values variant
    #[inline(always)]
    pub const fn variant(&self) -> CMPCHA23_A {
        match self.bits {
            false => CMPCHA23_A::_0,
            true => CMPCHA23_A::_1,
        }
    }
    ///Excludes AN023 from the compare window A target range.
    #[inline(always)]
    pub fn is_0(&self) -> bool {
        *self == CMPCHA23_A::_0
    }
    ///Includes AN023 from the compare window A target range.
    #[inline(always)]
    pub fn is_1(&self) -> bool {
        *self == CMPCHA23_A::_1
    }
}
///Field `CMPCHA23` writer - AN023 Select
pub type CMPCHA23_W<'a, REG> = crate::BitWriter<'a, REG, CMPCHA23_A>;
impl<'a, REG> CMPCHA23_W<'a, REG>
where
    REG: crate::Writable + crate::RegisterSpec,
{
    ///Excludes AN023 from the compare window A target range.
    #[inline(always)]
    pub fn _0(self) -> &'a mut crate::W<REG> {
        self.variant(CMPCHA23_A::_0)
    }
    ///Includes AN023 from the compare window A target range.
    #[inline(always)]
    pub fn _1(self) -> &'a mut crate::W<REG> {
        self.variant(CMPCHA23_A::_1)
    }
}
/**AN024 Select

Value on reset: 0*/
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum CMPCHA24_A {
    ///0: Excludes AN024 from the compare window A target range.
    _0 = 0,
    ///1: Includes AN024 from the compare window A target range.
    _1 = 1,
}
impl From<CMPCHA24_A> for bool {
    #[inline(always)]
    fn from(variant: CMPCHA24_A) -> Self {
        variant as u8 != 0
    }
}
///Field `CMPCHA24` reader - AN024 Select
pub type CMPCHA24_R = crate::BitReader<CMPCHA24_A>;
impl CMPCHA24_R {
    ///Get enumerated values variant
    #[inline(always)]
    pub const fn variant(&self) -> CMPCHA24_A {
        match self.bits {
            false => CMPCHA24_A::_0,
            true => CMPCHA24_A::_1,
        }
    }
    ///Excludes AN024 from the compare window A target range.
    #[inline(always)]
    pub fn is_0(&self) -> bool {
        *self == CMPCHA24_A::_0
    }
    ///Includes AN024 from the compare window A target range.
    #[inline(always)]
    pub fn is_1(&self) -> bool {
        *self == CMPCHA24_A::_1
    }
}
///Field `CMPCHA24` writer - AN024 Select
pub type CMPCHA24_W<'a, REG> = crate::BitWriter<'a, REG, CMPCHA24_A>;
impl<'a, REG> CMPCHA24_W<'a, REG>
where
    REG: crate::Writable + crate::RegisterSpec,
{
    ///Excludes AN024 from the compare window A target range.
    #[inline(always)]
    pub fn _0(self) -> &'a mut crate::W<REG> {
        self.variant(CMPCHA24_A::_0)
    }
    ///Includes AN024 from the compare window A target range.
    #[inline(always)]
    pub fn _1(self) -> &'a mut crate::W<REG> {
        self.variant(CMPCHA24_A::_1)
    }
}
/**AN025 Select

Value on reset: 0*/
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum CMPCHA25_A {
    ///0: Excludes AN025 from the compare window A target range.
    _0 = 0,
    ///1: Includes AN025 from the compare window A target range.
    _1 = 1,
}
impl From<CMPCHA25_A> for bool {
    #[inline(always)]
    fn from(variant: CMPCHA25_A) -> Self {
        variant as u8 != 0
    }
}
///Field `CMPCHA25` reader - AN025 Select
pub type CMPCHA25_R = crate::BitReader<CMPCHA25_A>;
impl CMPCHA25_R {
    ///Get enumerated values variant
    #[inline(always)]
    pub const fn variant(&self) -> CMPCHA25_A {
        match self.bits {
            false => CMPCHA25_A::_0,
            true => CMPCHA25_A::_1,
        }
    }
    ///Excludes AN025 from the compare window A target range.
    #[inline(always)]
    pub fn is_0(&self) -> bool {
        *self == CMPCHA25_A::_0
    }
    ///Includes AN025 from the compare window A target range.
    #[inline(always)]
    pub fn is_1(&self) -> bool {
        *self == CMPCHA25_A::_1
    }
}
///Field `CMPCHA25` writer - AN025 Select
pub type CMPCHA25_W<'a, REG> = crate::BitWriter<'a, REG, CMPCHA25_A>;
impl<'a, REG> CMPCHA25_W<'a, REG>
where
    REG: crate::Writable + crate::RegisterSpec,
{
    ///Excludes AN025 from the compare window A target range.
    #[inline(always)]
    pub fn _0(self) -> &'a mut crate::W<REG> {
        self.variant(CMPCHA25_A::_0)
    }
    ///Includes AN025 from the compare window A target range.
    #[inline(always)]
    pub fn _1(self) -> &'a mut crate::W<REG> {
        self.variant(CMPCHA25_A::_1)
    }
}
impl R {
    ///Bit 0 - AN016 Select
    #[inline(always)]
    pub fn cmpcha16(&self) -> CMPCHA16_R {
        CMPCHA16_R::new((self.bits & 1) != 0)
    }
    ///Bit 1 - AN017 Select
    #[inline(always)]
    pub fn cmpcha17(&self) -> CMPCHA17_R {
        CMPCHA17_R::new(((self.bits >> 1) & 1) != 0)
    }
    ///Bit 2 - AN018 Select
    #[inline(always)]
    pub fn cmpcha18(&self) -> CMPCHA18_R {
        CMPCHA18_R::new(((self.bits >> 2) & 1) != 0)
    }
    ///Bit 3 - AN019 Select
    #[inline(always)]
    pub fn cmpcha19(&self) -> CMPCHA19_R {
        CMPCHA19_R::new(((self.bits >> 3) & 1) != 0)
    }
    ///Bit 4 - AN020 Select
    #[inline(always)]
    pub fn cmpcha20(&self) -> CMPCHA20_R {
        CMPCHA20_R::new(((self.bits >> 4) & 1) != 0)
    }
    ///Bit 5 - AN021 Select
    #[inline(always)]
    pub fn cmpcha21(&self) -> CMPCHA21_R {
        CMPCHA21_R::new(((self.bits >> 5) & 1) != 0)
    }
    ///Bit 6 - AN022 Select
    #[inline(always)]
    pub fn cmpcha22(&self) -> CMPCHA22_R {
        CMPCHA22_R::new(((self.bits >> 6) & 1) != 0)
    }
    ///Bit 7 - AN023 Select
    #[inline(always)]
    pub fn cmpcha23(&self) -> CMPCHA23_R {
        CMPCHA23_R::new(((self.bits >> 7) & 1) != 0)
    }
    ///Bit 8 - AN024 Select
    #[inline(always)]
    pub fn cmpcha24(&self) -> CMPCHA24_R {
        CMPCHA24_R::new(((self.bits >> 8) & 1) != 0)
    }
    ///Bit 9 - AN025 Select
    #[inline(always)]
    pub fn cmpcha25(&self) -> CMPCHA25_R {
        CMPCHA25_R::new(((self.bits >> 9) & 1) != 0)
    }
}
impl W {
    ///Bit 0 - AN016 Select
    #[inline(always)]
    pub fn cmpcha16(&mut self) -> CMPCHA16_W<'_, ADCMPANSR1_SPEC> {
        CMPCHA16_W::new(self, 0)
    }
    ///Bit 1 - AN017 Select
    #[inline(always)]
    pub fn cmpcha17(&mut self) -> CMPCHA17_W<'_, ADCMPANSR1_SPEC> {
        CMPCHA17_W::new(self, 1)
    }
    ///Bit 2 - AN018 Select
    #[inline(always)]
    pub fn cmpcha18(&mut self) -> CMPCHA18_W<'_, ADCMPANSR1_SPEC> {
        CMPCHA18_W::new(self, 2)
    }
    ///Bit 3 - AN019 Select
    #[inline(always)]
    pub fn cmpcha19(&mut self) -> CMPCHA19_W<'_, ADCMPANSR1_SPEC> {
        CMPCHA19_W::new(self, 3)
    }
    ///Bit 4 - AN020 Select
    #[inline(always)]
    pub fn cmpcha20(&mut self) -> CMPCHA20_W<'_, ADCMPANSR1_SPEC> {
        CMPCHA20_W::new(self, 4)
    }
    ///Bit 5 - AN021 Select
    #[inline(always)]
    pub fn cmpcha21(&mut self) -> CMPCHA21_W<'_, ADCMPANSR1_SPEC> {
        CMPCHA21_W::new(self, 5)
    }
    ///Bit 6 - AN022 Select
    #[inline(always)]
    pub fn cmpcha22(&mut self) -> CMPCHA22_W<'_, ADCMPANSR1_SPEC> {
        CMPCHA22_W::new(self, 6)
    }
    ///Bit 7 - AN023 Select
    #[inline(always)]
    pub fn cmpcha23(&mut self) -> CMPCHA23_W<'_, ADCMPANSR1_SPEC> {
        CMPCHA23_W::new(self, 7)
    }
    ///Bit 8 - AN024 Select
    #[inline(always)]
    pub fn cmpcha24(&mut self) -> CMPCHA24_W<'_, ADCMPANSR1_SPEC> {
        CMPCHA24_W::new(self, 8)
    }
    ///Bit 9 - AN025 Select
    #[inline(always)]
    pub fn cmpcha25(&mut self) -> CMPCHA25_W<'_, ADCMPANSR1_SPEC> {
        CMPCHA25_W::new(self, 9)
    }
}
/**A/D Compare Function Window A Channel Select Register 1

You can [`read`](crate::Reg::read) this register and get [`adcmpansr1::R`](R). You can [`reset`](crate::Reg::reset), [`write`](crate::Reg::write), [`write_with_zero`](crate::Reg::write_with_zero) this register using [`adcmpansr1::W`](W). You can also [`modify`](crate::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api).*/
pub struct ADCMPANSR1_SPEC;
impl crate::RegisterSpec for ADCMPANSR1_SPEC {
    type Ux = u16;
}
///`read()` method returns [`adcmpansr1::R`](R) reader structure
impl crate::Readable for ADCMPANSR1_SPEC {}
///`write(|w| ..)` method takes [`adcmpansr1::W`](W) writer structure
impl crate::Writable for ADCMPANSR1_SPEC {
    type Safety = crate::Unsafe;
}
///`reset()` method sets ADCMPANSR1 to value 0
impl crate::Resettable for ADCMPANSR1_SPEC {}
