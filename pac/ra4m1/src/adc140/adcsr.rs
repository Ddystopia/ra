...
///Field `DBLANS` reader - Double Trigger Channel Select These bits select one analog input channel for double triggered operation. The setting is only effective while double trigger mode is selected.
pub type DBLANS_R = crate::FieldReader;
///Field `DBLANS` writer - Double Trigger Channel Select These bits select one analog input channel for double triggered operation. The setting is only effective while double trigger mode is selected.
...
pub enum GBADIE_A {
    ///0: Disables S12GBADI0 interrupt generation upon group B scan completion.
    _0 = 0,
    ///1: Enables S12GBADI0 interrupt generation upon group B scan completion.
    _1 = 1,
}
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum EXTRG_A {
    ///0: A/D conversion is started by the synchronous trigger (ELC).
    _0 = 0,
...
pub enum ADHSC_A {
    ...
    _0 = 0,
    ///1: Low current A/D conversion mode
    _1 = 1,
}
...
///Field `ADHSC` reader - A/D Conversion Operation Mode Select
...
///Field `ADHSC` writer - A/D Conversion Operation Mode Select
pub type ADHSC_W<'a, REG> = crate::BitWriter<'a, REG, ADHSC_A>;
impl<'a, REG> ADHSC_W<'a, REG>
where
    REG: crate::Writable + crate::RegisterSpec,
{
    ///High speed A/D conversion mode
...
impl R {
    ///Bits 0:4 - Double Trigger Channel Select These bits select one analog input channel for double triggered operation. The setting is only effective while double trigger mode is selected.
    #[inline(always)]
    ...
    ///Bit 10 - A/D Conversion Operation Mode Select
    #[inline(always)]
...
impl W {
    ...
    ///Bit 10 - A/D Conversion Operation Mode Select
    #[inline(always)]
    pub fn adhsc(&mut self) -> ADHSC_W<'_, ADCSR_SPEC> {
        ADHSC_W::new(self, 10)
    }
    ///Bits 13:14 - Scan Mode Select
    #[inline(always)]
